...
// 2. The function that gets injected into the webpage
const injectDialog = (passedSelection) => {
  // If triggered by shortcut, grab the highlighted text directly from the window
  const selectionText =
    passedSelection || window.getSelection().toString().trim();

  if (!selectionText) {
    alert(
      "ContextNote: Please highlight some text first before using the shortcut!",
    );
    return;
  }
  ...
  document.getElementById("cn-save").onclick = () => {
    const title =
      document.getElementById("cn-title").value.trim() || "Highlighted Text";
...
// 4. Trigger via Keyboard Shortcut (Alt+Shift+S)
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (tabs[0]) {
        chrome.scripting.executeScript({
          ...
          args: [null], // Pass null so the script grabs the selection from the window directly
          func: injectDialog,
        });
      }
    });
...
// 5. Listener: Receives the note and saves it to Chrome Local Storage
  ...
  if (request.action === "save_highlight_data") {
    chrome.storage.local.get(STORAGE_KEY, (res) => {
      ...
      chrome.storage.local.set({ [STORAGE_KEY]: JSON.stringify(notes) }, () => {
        // TELL THE WEBPAGE TO UPDATE ITS HIGHLIGHTS IMMEDIATELY
        if (sender.tab) {
